...
@Data
@ToString
@ApiModel(value = "InsertAdminInfoDto", description = "添加管理员信息参数")
public class InsertAdminInfoDto implements Serializable {

    private static final long serialVersionUID = -162232796878583717L;

    @NotBlank(message = "管理员姓名不能为空！")
    @Length(min = 2, max = 6, message = "管理员姓名长度必须在2~6位之间！")

    @ApiModelProperty(name = "adminName", value = "管理员姓名", dataType = "String", required = true)
    private String adminName;

    @NotBlank(message = "登录名不能为空！")
    @Length(min = 4, max = 20, message = "登录名长度必须在4 ~20位之间！")
    @Pattern(regexp = "^[a-z]+$", message = "登录名必须为全小写字母！")
